...
object ExtractLSVs extends Tool {
  ...
  def extractLSVs(vcfFiles: List[String], _referenceGenome: java.io.File, outputDir: String,
    ...
    vcfFiles.foreach(file => {
      ...
      LSVs.foreach { lsv =>
        val flanks = getFlankingSeqs(mapScaffoldtoGenome(lsv.chromosome))(lsv)
        if(flanks.size>0)
        	pw_all_LSVs.println(extendLSV(lsv, flanks, emu_sample_name))
        else println("WARNING: LSV ignored because to close to edge of the genome!!!")
      }
